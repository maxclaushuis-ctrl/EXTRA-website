...
const COPY = {
  NL: {
    ...
    noLanguage: "Jammer! Bij bijna al onze opdrachten heb je Nederlands of Engels nodig om goed met gasten en collega's te kunnen communiceren. Verbeter je je taalskills? Dan staan de deuren weer open.",
    ...
    notEnoughExperience: "Op dit moment zoeken onze opdrachtgevers kandidaten met wat meer werkervaring in deze functie. Dat zegt niks over jouw potentieel — het matcht nu gewoon nog niet. Kom gerust terug als je meer uren hebt staan!",
    roleNotAvailable: "Voor deze functie zijn we helaas gebonden aan Nederlandse wetgeving rondom werkvergunningen. Dat heeft niks met jou persoonlijk te maken — het is puur een juridische beperking.",
  ...
  EN: {
    ...
    notEnoughExperience: "Right now our clients are looking for candidates with a bit more experience in this role. That's no reflection on you — it just isn't the right match yet. Come back when you've got more hours under your belt!",
...
export default function Aanmelden() {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      isBarista: "" as "" | "ja" | "nee",
      canMakeCocktails: "" as "" | "ja" | "nee",
      isAssistantChef: "" as "" | "ja" | "nee",
      canDoWashing: "" as "" | "ja" | "nee",
      isPromoter: "" as "" | "ja" | "nee",
      serviceSkills: 0,
      barSkills: 0,
      dinerSkills: 0,
    };
  ...
  function horecaTagsPayload() {
    return {
      canIndependentShift: formData.canIndependentShift === "ja",
      canCarryThreePlates: formData.canCarry3Plates === "ja",
      isBarista: formData.isBarista === "ja",
      canMakeCocktails: formData.canMakeCocktails === "ja",
      isAssistantChef: formData.isAssistantChef === "ja",
      canDoWashing: formData.canDoWashing === "ja",
      isPromoter: formData.isPromoter === "ja",
      serviceSkills: formData.serviceSkills || null,
      barSkills: formData.barSkills || null,
      dinerSkills: formData.dinerSkills || null,
    };
  }
    ...
    if (detectedFlow === "NON_EU") {
      ...
      if (fn === "horecamedewerker") {
        ...
        const NL_SPEAKING_COUNTRIES = [
          ...
          "Suriname", "Curaçao", "Aruba", "Sint Maarten", "Bonaire"
        ];
        const isNlSpeaker = NL_SPEAKING_COUNTRIES.includes(formData.nationality);
  ...
  async function finalizeSubmission(twvData?: { needsTwv: boolean }) {
    ...
    const horecaTags = formData.preferredFunction === "horecamedewerker" ? horecaTagsPayload() : {};
