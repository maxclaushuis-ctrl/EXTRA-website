...
const tasks = [
  { icon: "🛏️", title: "Room cleaning", desc: "Full service turnover of guest rooms to brand standard." },
  { icon: "🧹", title: "Public areas", desc: "Lobby, corridors, restaurants and conference spaces." },
  { icon: "🛁", title: "Deep cleaning", desc: "Thorough cleans of rooms, bathrooms and suites." },
  { icon: "🧺", title: "Linen handling", desc: "Stripping, counting and organising linens for laundry." },
  { icon: "🏨", title: "Turndown service", desc: "Evening preparation of rooms for overnight guests." },
  { icon: "✅", title: "Inspection", desc: "Quality-checking rooms before guest check-in." },
];

const faq = [
  { q: "Do I need housekeeping experience?", a: "Experience is useful but not required. We train you on hotel standards and pair you with experienced colleagues on your first shifts." },
  { q: "What hours are typical?", a: "Most housekeeping shifts run from 08:00 to 16:00. We also have evening and weekend positions available." },
  { q: "Which hotels will I work at?", a: "We place housekeepers at Marriott, Hilton, NH, Mercure, Amrâth and many other Amsterdam hotels." },
  { q: "Is there a minimum number of shifts per week?", a: "No. You set your own availability. Some staff work two shifts a week, others work full time." },
];
...
export default function HousekeepingJobsEn() {
  ...
  useEffect(() => {
    document.title = "Housekeeping Jobs Amsterdam | EXTRA Hotel Staffing";
    ...
    s('description', 'Housekeeping jobs at top Amsterdam hotels. Work as a room attendant or public area cleaner. Formal contract, flexible hours and rewards for every shift. Apply at EXTRA.');
    l('canonical', 'https://www.doehetextra.nl/en/housekeeping-jobs');
    l('alternate', 'https://www.doehetextra.nl/housekeeping-vacatures-amsterdam', 'nl');
    l('alternate', 'https://www.doehetextra.nl/en/housekeeping-jobs', 'en');
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="relative min-h-[70vh] flex items-center overflow-hidden">
        <div className="absolute inset-0"><img src={housekeepingImg} alt="Housekeeping professional" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-[#1a0a2e]/95 via-[#170926]/80 to-transparent" /></div>
        <div className="relative z-10 max-w-5xl mx-auto px-5 sm:px-8 py-32 sm:py-40">
          <RevealSection>
            ...
            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-white mb-6 leading-[1.06]" style={{ fontFamily: "'Poppins', sans-serif" }}>
              Housekeeping jobs<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">at top hotels</span>
            </h1>
            <p className="text-lg sm:text-xl text-purple-100/80 max-w-xl mb-10 leading-relaxed">Room attendant, public area cleaner or turndown staff. Formal contract, same-day pay and a rewards system that appreciates your hard work.</p>
      ...
      <div className="bg-gray-50 border-y border-gray-100 py-10">
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          <p className="text-center text-xs text-gray-400 uppercase tracking-widest font-bold mb-6">Work at Amsterdam's best hotels</p>
          <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12">{logos.map((logo, i) => <img key={i} src={logo} alt="Client logo" className="h-7 sm:h-8 object-contain opacity-40 hover:opacity-70 transition-opacity grayscale" loading="lazy" />)}</div>
        </div>
      </div>
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8 relative z-10">
          <RevealSection><div className="text-center mb-14"><span className="text-purple-600 text-sm font-bold uppercase tracking-widest">Types of work</span><h2 className="text-3xl sm:text-5xl font-black text-gray-900 mt-3" style={{ fontFamily: "'Poppins', sans-serif" }}>What you will do</h2></div></RevealSection>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{tasks.map((t, i) => (<RevealSection key={i} delay={i * 60}><div className="bg-blue-50 rounded-2xl p-6 border border-blue-100"><span className="text-3xl mb-3 block">{t.icon}</span><h3 className="font-black text-gray-900 text-base mb-2">{t.title}</h3><p className="text-sm text-gray-500 leading-relaxed">{t.desc}</p></div></RevealSection>))}</div>
        </div>
      </section>
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8 relative z-10">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{[
            { icon: CheckCircle2, label: "Formal contract", desc: "Properly employed. No zero-hours uncertainty." },
            { icon: Banknote, label: "Same-day pay", desc: "Paid instantly after each shift via the app." },
            { icon: Gift, label: "Rewards every shift", desc: "Earn points. Redeem for real prizes." },
            { icon: CalendarCheck, label: "Flexible hours", desc: "You choose your availability. We find matching shifts." },
            { icon: Trophy, label: "Recognition", desc: "Monthly leaderboard. Best performers get bonuses." },
            { icon: Zap, label: "Quick to start", desc: "Apply online. Start within days." },
          ].map((f, i) => (<RevealSection key={i} delay={i * 60}><div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/15"><f.icon className="w-8 h-8 text-blue-300 mb-3" /><h3 className="font-black text-white text-sm mb-1.5">{f.label}</h3><p className="text-white/60 text-sm leading-relaxed">{f.desc}</p></div></RevealSection>))}</div>
        </div>
      </section>
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-5 text-center relative z-10">
          <RevealSection>
            <h2 className="text-3xl sm:text-4xl font-black text-white mb-5" style={{ fontFamily: "'Poppins', sans-serif" }}>Ready to start?</h2>
            <p className="text-purple-200/80 text-lg mb-10">Apply in 5 minutes. We will be in touch shortly.</p>
