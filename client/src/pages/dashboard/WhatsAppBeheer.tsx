...
import { haalAccountsOp, haalBerichtenOp, stuurBericht } from '../../api/whatsappClient';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: status === 'connected' ? '#D1FAE5' : '#F3F4F6',
          color: status === 'connected' ? '#065F46' : '#6B7280',
        }}>
          {status === 'connected' ? 'Actief' : 'Inactief'}
        </div>
      </div>
      ...
      <div style={{ padding: '14px 24px', borderTop: '1px solid #EEEEEE', flexShrink: 0, background: '#FAFAFA' }}>
        ...
        <p style={{ fontSize: 10, color: '#C4BED8', margin: '6px 0 0' }}>
          Inkomende berichten komen binnen via de 360dialog webhook → <code style={{ fontSize: 10 }}>/api/whatsapp/webhook</code>
        </p>
      </div>
    </div>
  );
