...
import blogHousekeepingImg from "@/assets/images/blog-housekeeping.webp";
import dienstHousekeepingImg from "@/assets/images/dienst-housekeeping.webp";
...
const functies = [
  {
    ...
    sub: "Hotelkamers · Gastvrijheid · Precisie",
    img: housekeepingImg,
    desc: "Zorg voor een onberispelijk schoone hotelkamer. Jij bepaalt de eerste indruk van de gast bij aankomst.",
    bullets: ["Dagdiensten in tophotels", "Structuur en precisie", "Vaste poule per hotel"],
  ...
  },
  {
    ...
    desc: "Algehele schoonmaak in hotelomgevingen. Van kamers tot gemeenschappelijke ruimtes en gangpaden.",
    bullets: ["Gevarieerd werkpakket", "Internationale hotelketens", "Goede dagindeling"],
    color: "from-teal-500 to-cyan-600",
    href: "/aanmelden",
  },
  {
    title: "Public Area Cleaning",
    sub: "Lobby · Gangen · Restaurants",
    img: dienstHousekeepingImg,
    desc: "Schoonmaak van openbare ruimtes zoals de lobby, liften, gangen en hotelrestaurant. Altijd representatief.",
    bullets: ["Ochtend- en avonddiensten", "Hoog tempo en zichtbaar resultaat", "Zelfstandig werken"],
    color: "from-violet-500 to-purple-700",
    href: "/aanmelden",
  },
  {
    title: "Linnen Service",
    sub: "Linnenkamer · Was · Logistiek",
    img: hotelImg,
    desc: "Beheer en verzorging van hotellinnen. Van sorteren en wassen tot distributie op de etages.",
    bullets: ["Georganiseerd werktempo", "Backstage in tophotels", "Teamwerk centraal"],
...
const reviews = [
  { name: "Fatima A.", functie: "Room Attendant", tekst: "Via EXTRA werk ik in mooie hotels en weet ik altijd waar ik aan toe ben. De planning is duidelijk en het team is fijn.", rating: 5 },
  { name: "Nadia R.", functie: "Hotel schoonmaak", tekst: "Dagbetaling via Jixbee maakt het verschil. Ik zie gelijk wat ik verdien en het staat er snel op. Dat geeft rust.", rating: 5 },
  { name: "Ingrid M.", functie: "Public Area Cleaning", tekst: "Ik werk nu een half jaar via EXTRA. De locaties zijn netjes, het werk is duidelijk en ik kan zelf kiezen wanneer ik ga.", rating: 5 },
];

const faqs = [
  { q: "Heb ik ervaring nodig voor housekeeping werk?", a: "Ervaring is een pluspunt, maar niet altijd vereist. We kijken vooral naar jouw motivatie, nauwkeurigheid en instelling. Voor sommige functies bieden we een korte introductie op locatie aan." },
  { q: "Werk ik altijd in hetzelfde hotel?", a: "In de meeste gevallen ja. We proberen je te koppelen aan een vaste poule bij een of meerdere hotels, zodat je de werkwijze kent en je snel thuis voelt in het team." },
  { q: "Kan ik zelf mijn diensten kiezen?", a: "Ja. Je geeft aan wanneer je beschikbaar bent en wij zorgen voor passende diensten. Zo combineer je het werk met andere verplichtingen of activiteiten." },
  { q: "Hoe werkt de betaling?", a: "Na je dienst word je uitbetaald via Jixbee. Het bedrag staat doorgaans dezelfde dag nog op je rekening. Transparant en snel, zonder verrassingen." },
  { q: "Hoe snel kan ik beginnen met werken?", a: "Na je aanmelding en een korte kennismaking kun je vaak al binnen een week je eerste diensten pakken. We zorgen dat alles snel en soepel verloopt." },
];
...
export default function HousekeepingWerk() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center gap-2">
                ...
                <div className={`absolute top-full left-0 pt-2 transition-all duration-200 ${activeDropdown === "werk" ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-2 pointer-events-none"}`}>
                  <div className="bg-white rounded-2xl shadow-2xl shadow-purple-500/10 border border-purple-100/60 p-2 min-w-[220px]">
                    {[
                      ...
                      <a key={item.label} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all group ${item.href === "/housekeeping-werk" ? "bg-purple-50 text-purple-700" : "text-gray-700 hover:bg-purple-50 hover:text-purple-700"}`}>
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${item.href === "/housekeeping-werk" ? "bg-indigo-200" : "bg-indigo-100 group-hover:bg-indigo-200"}`}>
              ...
              <a href="/onze-werkwijze" className={`flex items-center gap-2 text-[18px] font-bold px-5 py-3 rounded-lg transition-all ${scrolled ? "text-gray-800 hover:text-purple-600 hover:bg-purple-50/50" : "text-white/90 hover:text-white hover:bg-white/10"}`}>
                <Clock className="w-5 h-5" /> Werkwijze
              </a>
              <a href="/ons-team" className={`flex items-center gap-2 text-[18px] font-bold px-5 py-3 rounded-lg transition-all ${scrolled ? "text-gray-800 hover:text-purple-600 hover:bg-purple-50/50" : "text-white/90 hover:text-white hover:bg-white/10"}`}>
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${mobileMenuOpen ? "max-h-[80vh] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="bg-white border-t border-gray-100 shadow-2xl overflow-y-auto max-h-[80vh]">
            <div className="px-5 py-5 space-y-1">
                ...
                <div className={`overflow-hidden transition-all duration-300 ${mobileExpanded === "werk" ? "max-h-60" : "max-h-0"}`}>
                  <div className="pl-16 pr-4 pb-2 space-y-0.5">
                    {[
                      ...
                      <a key={item.label} href={item.href} className={`block py-2.5 text-sm font-medium transition-colors ${item.href === "/housekeeping-werk" ? "text-purple-600 font-bold" : "text-gray-600 hover:text-purple-600"}`}>{item.label}</a>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <a href="/onze-werkwijze" className="flex items-center gap-3 px-4 py-3.5 rounded-xl text-gray-800 font-bold text-base hover:bg-purple-50 transition-colors">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden" style={{ background: "linear-gradient(135deg, #2d0663 0%, #4a0e96 35%, #5b16a8 65%, #6d28d9 100%)" }}>
        ...
        <div className="absolute inset-0 pointer-events-none">
          <img
            ...
            alt="Housekeeping medewerker via EXTRA – kamermeisje tophotel Amsterdam"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 pt-28 pb-20 sm:pt-32 sm:pb-24 w-full">
          <div className="max-w-xl lg:max-w-[52%] 2xl:max-w-[42%]">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-1.5 mb-7 border border-white/20">
              ...
              <span className="text-white/90 text-xs font-semibold">Housekeeping vacatures via EXTRA</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-[4.25rem] text-white leading-[1.05] mb-6" style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 900 }}>
              ...
              <span className="relative inline-block">
                <span className="relative z-10">Jij vliegt erdoorheen.</span>
            ...
            <p className="text-lg sm:text-xl text-purple-100/90 leading-relaxed font-medium mb-8 max-w-lg">
              Werk als kamermeisje of housekeeping medewerker bij tophotels in Amsterdam. Via EXTRA kies je zelf je diensten. Flexibel werk met dagbetaling en professionele hotelomgevingen.
            </p>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 sm:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-7">
            {[
              ...
              {
                icon: Shield, badge: "Zekerheid", title: "Iedereen in loondienst",
                desc: "Je werkt altijd met een officieel contract via EXTRA. Geen grijze constructies, gewoon eerlijk en transparant.",
                bg: "from-green-50 to-emerald-50", border: "border-green-200", iconBg: "bg-green-100", iconColor: "text-green-600", tag: "✅ Veilig en eerlijk",
              },
              {
                icon: Gift, badge: "Exclusief beloningssysteem", title: "EXTRAATJE beloningen",
                desc: "Spaar punten voor elke dienst en wissel ze in voor gave beloningen. Van cadeaubonnen tot gadgets.",
              ...
              },
              {
                ...
                desc: "Onze planners kennen je naam en situatie. Snel bereikbaar en altijd eerlijk over beschikbare diensten.",
      ...
      <section className="py-16 sm:py-24 lg:py-28" style={{ background: "linear-gradient(135deg, #f9f7ff 0%, #ffffff 100%)" }}>
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-5 sm:gap-6 mb-10">
            {[
              { icon: Building2, title: "Vijfsterren hotels", desc: "Werk bij internationale hotelketens in Amsterdam, Utrecht en Den Haag. Topstandaarden en professionele teams.", iconBg: "bg-indigo-100", iconColor: "text-indigo-600", bg: "from-indigo-50 to-blue-50", border: "border-indigo-200" },
              { icon: Sparkles, title: "Boutique hotels", desc: "Kleinere, persoonlijkere hotelomgevingen met een hoge kwaliteitsstandaard en aandacht voor detail.", iconBg: "bg-purple-100", iconColor: "text-purple-600", bg: "from-purple-50 to-violet-50", border: "border-purple-200" },
              { icon: Shield, title: "Vaste hotelpoule", desc: "Je wordt gekoppeld aan een vaste poule zodat je de werkwijze kent, vertrouwd bent met het team en snel inzetbaar bent.", iconBg: "bg-emerald-100", iconColor: "text-emerald-600", bg: "from-emerald-50 to-teal-50", border: "border-emerald-200" },
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-5 sm:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
            {[
              ...
              { step: "02", icon: Users, title: "Kennismaking", desc: "We plannen een kort gesprek. Dan kijken we bij welke hotels je het beste past.", color: "from-indigo-500 to-blue-600" },
              { step: "03", icon: CalendarCheck, title: "Kies je diensten", desc: "Jij geeft je beschikbaarheid door en wij zorgen voor passende housekeeping diensten.", color: "from-teal-500 to-cyan-600" },
              { step: "04", icon: Zap, title: "Werk en word betaald", desc: "Draai je dienst en ontvang dezelfde dag je betaling via Jixbee.", color: "from-orange-500 to-amber-500" },
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          <RevealSection>
            <div className="text-center mb-10 sm:mb-14">
              ...
              <h2 className="text-3xl sm:text-5xl font-black text-gray-900 mt-3 leading-tight" style={{ fontFamily: "'Poppins', sans-serif" }}>
                Wat zeggen housekeeping medewerkers?
              </h2>
            </div>
          </RevealSection>
      ...
      {/* ══ 10. FAQ ══ */}
      <FAQSection
        heading="Veelgestelde vragen over housekeeping werk via EXTRA"
        faqs={faqs}
      />
